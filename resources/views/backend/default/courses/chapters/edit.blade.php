...
@section('content')
    <div class="page-header">
        <div class="row align-items-center bg-white p-2">
            <div class="col">
                <h4 class="text-capitalize mt-3 mb-0">Edit Chapter</h4>
            </div>
    ...
    {{ Form::open(['method' => 'PUT', 'route' => ['chapter.update', $chapter], 'files' => 'true', 'enctype' => 'multipart/form-data',]) }}
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group">
                                <label>Title</label>
                                <input type="text" name="title" class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" value="{{ $chapter->title }}" required>
                                @if ($errors->has('title'))
                            ...
                            <div class="form-group">
                                ...
                                <select name="course" class="form-control form-select {{ $errors->has('course') ? 'is-invalid' : '' }}" required>
                                    ...
                                    @foreach($courses as $course)
                                        <option value="{{ $course->id  }}" {{ ($chapter->course_id == $course->id) ? 'selected' : '' }}>{{$course->title }} </option>
                                    @endforeach
                                </select>
